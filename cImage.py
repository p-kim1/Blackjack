...
class AbstractImage(object):
        ...
        if fname:
            ...
            for row  in range(height):
                for col in range(width):
                    # AJW Fixed to get RGB values
                    red = data[row][col][0]
                    green = data[row][col][1]
                    blue = data[row][col][2]
                    self.setPixel(col,row,Pixel(red, green, blue))
    ...
    def createBlankTkImage(self,height,width):
        ...
        # AJW set width and height so setTKPixel has it
        self.width = self.im.width()
        self.height = self.im.height()
    ...
    def getTkPixel(self,x,y):
        ...
        # AJW fix for change in representation between 3.2 and 3.4
        try:
